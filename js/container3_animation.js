...
var dataz=["REWARDSZ", "SAVINGSZ" , "INVESTMENTSZ", "FOREXZ", "ANALYTICSZ", "CARDSZ" , "PAYMENTSZ", "LOANZ", "SUPPORTZ"]
...
function myTimer() 
{
    ...
    document.getElementById(dataz[i]).src = large[i]; 
    for(j=0;j<datax.length;j++)
    {
        if(j!=i)
        {
          ...
          document.getElementById(dataz[j]).src = small[j];
        }
    }
